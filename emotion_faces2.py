...
colors = {'Neutralny': (255, 255, 255), 'Zlosc': (0, 0, 255), 'Strach': (0, 0, 0),
          'Szczescie': (0, 255, 255), 'Smutek': (255, 0, 0), 'Zaskoczenie': (255, 245, 0)}

imotions = {0: 'Zlosc', 1: 'Strach', 2: 'Szczescie', 3: 'Smutek',
            4: 'Zaskoczenie', 5: 'Neutralny'}
# imotions = {0:'Neutralny', 1:'Szczescie', 2:'Smutek', 3:'Zlosc',
#                4:'Zaskoczenie', 5:'Wstret', 6:'Strach'}
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        for i in range(len(pr)):
            cv.rectangle(img, (x, y + h + counter + 5), (x + int(w * pr[i]), y + h + counter + 20), colors[imotions[i]],
                         -2)

            counter += 20
            cv.putText(img, str(int(pr[i] * 100)), (x + int(w * pr[i]), (y + h + counter + 5)), cv.FONT_HERSHEY_SIMPLEX,
                       0.50, (0, 0, 0), 1)
            if i != 5:
